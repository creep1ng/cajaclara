...
class OCRService:
    ...
    async def process_receipt_image(
        ...
        try:
            ...
            raise OcrProcessingError(code="OCR_PROCESSING_ERROR", message=f"Error procesando imagen: {str(e)}")
    ...
    async def _validate_image(self, image_data: bytes) -> None:
        ...
        if len(image_data) > self.max_image_size:
            raise OcrProcessingError(
                ...
                message=
                f"Imagen demasiado grande. Máximo permitido: {settings.OCR_MAX_IMAGE_SIZE_MB}MB"
            )
        ...
        try:
            ...
            raise OcrProcessingError(code="INVALID_IMAGE_ERROR", message=f"Imagen inválida o corrupta: {str(e)}")
    ...
    async def _extract_text_with_openai(self, base64_image: str) -> str:
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                raise OcrProcessingError(code="OCR_SERVICE_ERROR", message="Error en el servicio de OCR")
                
            result = response.json()
            extracted_text = result["choices"][0]["message"]["content"].strip()
            
            if not extracted_text:
                raise OcrProcessingError(code="NO_TEXT_EXTRACTED", message="No se pudo extraer texto de la imagen")
                
            return extracted_text
            
        except httpx.RequestError as e:
            logger.error(f"Error llamando OpenAI API: {str(e)}")
            raise OcrProcessingError(code="OCR_CONNECTION_ERROR", message="Error de conexión con el servicio OCR")
        except Exception as e:
            logger.error(f"Error inesperado en OCR: {str(e)}")
            raise OcrProcessingError(code="OCR_UNKNOWN_ERROR", message=f"Error procesando OCR: {str(e)}")
        ...
        for pattern in patterns:
            ...
            for match in matches:
                try:
                    ...
                    if len(amount_str) > 2:  # Manejar centavos
    ...
    def _extract_date(self, text: str) -> Tuple[Optional[str], float]:
        ...
        patterns = [
            ...
            r'Fecha[:\s]*(\d{1,2})[/\-](\d{1,2})[/\-](\d{2,4})',  # Fecha: 27/10/2025
        ]
