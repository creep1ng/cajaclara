...
class Transaction(Base, UUIDMixin, TimestampMixin, SoftDeleteMixin, AuditMixin):
    ...
    # Transaction Data
    amount = Column(Numeric(15, 2), nullable=False, comment="Monto de la transacción")

    currency = Column(
        String(3), default="COP", nullable=False, comment="Moneda (ISO 4217)"
    )

    description = Column(Text, nullable=True, comment="Descripción o nota")

    transaction_type = Column(
        String(20), nullable=False, comment="Tipo: income o expense"
    )

    classification = Column(
        String(20), nullable=False, comment="Clasificación: personal o business"
    )
    ...
    # Additional Data
    tags = Column(ARRAY(Text), nullable=True, comment="Etiquetas adicionales")
    ...
    entrepreneurship = relationship("Entrepreneurship", back_populates="transactions")
